...
--we made the most money. Write a query that returns one city that has highest sum of invoices totals. 
...
--Q6 : Let's invite the artist who have written the most rock music in our dataset. Write a query 
...
-- This one is also work but if any new record will add or delete then you will change this value 
-- after every changes otherwise the output comes wrong.
...
--Q9 : We want to find out the most popular Music Genre for each country. We determine the most popular genre as the genre with 
--the highest amount of the purchases. Write a query that returns each country along with the top Genre. For countries where the 
